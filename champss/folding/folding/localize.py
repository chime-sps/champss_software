...
def beam_transit_model(t_transit, ra, dec, beamarg, spectrum=[], nchan=2048):
    """
    This function computes the frequency-averaged beam model I(t) for a transit at a given 
    beam number, for a given RA, DEC, and source spectrum.
...
def beam_dec_model(t_transit, ra, dec, A=1, nchan=2048):
    ...
    for i,beamarg in enumerate(beams):
        ...
        model_ts = beam_transit_model(t_transit, ra, dec, beamarg=beamarg, spectrum=spec)
        S = np.sum(model_ts)
        SNs[i] = S
...
def main(ra, dec, date, nday):
    ...
    Requisite is to have folded the data for the candidate in the max beam,
    as well as the beam above and below in declination.

    All archives must have the same ephemeris / parfile installed, there must be 
    slices of .F files for each day, and a combined archive added.T for each beam
    which has units of (time(days), pol, freq, phase), (where pol=1 is a dummy axis)
    """
    ...
    for i,dra in enumerate(dra_range):
        rai = ra + dra
        model_ts = beam_transit_model(t_transit, rai, dec, spectrum=spec, beamarg=beamarg)
