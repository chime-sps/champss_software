...
def create_report_pdf(date, db_host, db_port, db_name, basepath):
    ...
    all_obs = list(db.observations.find({"datetime": {"$gt": date, "$lt": date + datetime.timedelta(days=1)}}))
    all_obs_pointings = list([db.pointings.find_one({"_id": obs["pointing_id"]}) for obs in all_obs])
    all_obs_ra = [point["ra"] for point in all_obs_pointings]
    all_obs_dec = [point["dec"] for point in all_obs_pointings]
    plotted_quantities = ["status", "status", "mask_fraction", "num_detections", "num_clusters"]
    quantity_source = ["Process", "Observation", "Observation", "Observation","Observation"]
    ...
    with PdfPages(file_name) as pdf:
        try:
            ...
            for index, quant, source in zip(np.arange(rows), plotted_quantities, quantity_source):
                if source == "Process":
                    all_vals = np.array([proc.get(quant) for proc in all_proc]).astype(float)
                    ra = [proc.get("ra") for proc in all_proc]
                    dec = [proc.get("dec") for proc in all_proc]
                else:
                    all_vals = np.array([obs.get(quant) for obs in all_obs]).astype(float)
                    ra = all_obs_ra
                    dec = all_obs_dec
                ra_dec_index = 0
                status_hist_index = 1
                if quant == "status":
                    im1 = axs[index,ra_dec_index].scatter(ra, dec, c=all_vals, alpha=1, vmin=0, vmax=5, cmap=plt.cm.jet)
                    hist = axs[index,status_hist_index].hist(all_vals, bins= np.arange(7)-0.5)
                else:
                    im1 = axs[index,ra_dec_index].scatter(ra, dec, c=all_vals, alpha=0.5)
            ...
            pass
        try:
            df = pd.read_csv(daily_run.classification_result["output_file"], index_col=0)
            fig2, axs = plt.subplots(2,1, figsize=(12,12), rasterized=True)
            fig2.suptitle(f"Quality Report {date_string} Multi-Pointing Metrics")
            col1 = 'best_ra'
            col2 = 'best_dec'
            z = 'sigma'
            axs[0].scatter(df[col1], df[col2], c=np.log10(df[z]), alpha=0.05, marker='.')
            ...
            col2 = 'mean_dm'
            z = 'sigma'
            axs[1].scatter(df[col1], df[col2], c=np.log10(df[z]), alpha=0.05, marker='.')
