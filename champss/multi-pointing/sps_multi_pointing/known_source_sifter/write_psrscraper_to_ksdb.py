...
if __name__ == "__main__":
    ...
    for i in range(len(data["PSR"])):
        ...
        if (
            dec > -20 and survey != "ATNF"
        ):  # ATNF sources are redundant with the write_psrcat script
            duplicate = check_if_duplicate(
                psrname, ra, dec, P0, DM, [survey.lower(), "psr_scraper"]
            )
            if duplicate:
                ks = db.known_sources.find_one(
                    {"source_name": psrname, "survey": [survey.lower(), "psr_scraper"]}
                )
                if ks:
                ...
                payload = {
                    ...
                    "pos_ra_deg": ra,
                    "pos_dec_deg": dec,
                    "pos_error_semimajor_deg": np.nan,
                    "pos_error_semiminor_deg": np.nan,
                    "pos_error_theta_deg": 0.0,
                    "dm": DM,
                    "dm_error": np.nan,
                    "spin_period_s": P0,
                    "spin_period_s_error": np.nan,
