...
if __name__ == "__main__":
    """
    This script adds all pulsars in the pulsarsurveyscraper into the known_source database

    https://github.com/dlakaplan/pulsarsurveyscraper
    """
    parser = argparse.ArgumentParser(
        description=(
            "Adding known sources to the database"
        )
    )
    parser.add_argument(
        "--db-port",
        ...
        if dec > -20:
            payload = {
                ...
                "dm_galactic_ne_2001_max": float(
                    dmm.get_dm_ne2001(dec, ra)
                ),
                "dm_galactic_ymw_2016_max": float(
                    dmm.get_dm_ymw16(dec, ra)
                ),
                "spin_period_derivative": 0.0,
                "spin_period_derivative_error": 0.0,
                "spin_period_epoch": 45000., #placeholder, older than psrcat so as not to overwrite
            }
