...
from sps_common.interfaces import DedispersedTimeSeries, SkyBeam
from sps_dedispersion.fdmt.cpu_fdmt import FDMT


def simple_dedisperse(fdmt, skybeam, dm_step=1):
    """
    Dedisperse the skybeam into a collection of dedispersed time series without
    chunking.
    ...
    dms = (
        np.arange(0, fdmt.maxDT, dm_step)
        / DM_CONSTANT
        / (1 / fdmt.fmin**2 - 1 / fdmt.fmax**2)
    ) * TSAMP
...
def dedisperse(fdmt, skybeam, chunk_size, dm_step=1):
    ...
    # Chunking setup.
    dedisp = np.zeros(
        ((fdmt.maxDT - 1) // dm_step + 1, spectra.shape[1] - fdmt.maxDT),
        dtype=spectra.dtype,
    )
    ...
    while (idx + 1) * chunk_size + fdmt.maxDT < spectra.shape[1]:
        ...
        if idx == 0:
            ...
            np.add(
                dedisp_chunk,
                fdmt.fdmt(spectra_chunk, padding=True, frontpadding=False)[::dm_step],
                out=dedisp_chunk,
            )
        else:
            dedisp_chunk = dedisp[
                :, idx * chunk_size - fdmt.maxDT : (idx + 1) * chunk_size
            ]
