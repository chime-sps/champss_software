"""Reads in a single binary .dat waterfall from stdin and writes out the dispersion
transform.
"""
...
def do_fdmt_from_stdin(fdmt, nbytes, nsamps, maxsamps, dtype, verbosity=0):
    """
...
def display_top(snapshot, key_type="lineno", limit=3, verbosity=0):
    ...
    if other:
        ...
        print(f"{len(other)} other: {size / 1024:.1f} KiB", file=out)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "nchans",
        type=int,
        help="the number of frequency channels in the data in the file (default=1024)",
    )
    parser.add_argument(
        "nbytes", type=int, help="the number of bytes per data point in the file (4)"
    )
    parser.add_argument(
        "nsamps", type=int, help="the number of samples to read at a time (8192)"
    )
    parser.add_argument(
        "maxdm",
        type=int,
        help=(
            "the maximum DM to which "
    ...
    parser.add_argument(
        ...
        type=int,
        help=(
            "the maximum number of samples to read; or -1 to read the entire file. (-1)"
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="if present, check FDMT memory usage periodically",
    )
    ...
    if args.memory_check:
        ...
        monitor_thread = Thread(
            target=memory_monitor, args=(queue, poll_interval, args.verbosity)
        )
        monitor_thread.start()

    try:
        do_fdmt_from_stdin(
            fdmt, args.nbytes, args.nsamps, args.maxsamps, dtype, args.verbosity
        )
    finally:
        if args.memory_check:
            queue.put("stop")
