    ...
    def make_single_pointing_candidate(
        ...
        if self.write_detections_to_candidates:
            written_detections = rfn.structured_to_unstructured(
                cluster.detections[
                    ["dm", "freq", "sigma", "nharm", "injection", "convolve"]
                ]
            )
        else:
            ...
            for factor_index, factor in enumerate(used_factors):
                ...
                if factor == 1:
                    ...
                    if cluster.convolve > 1:
                        ...
                        prep_convolve = pss.prepare_unique_harmonic_bins(
                            full_harm_bins[
                                : int(best_nharm),
                                f0_idx_min_convolve:f0_idx_max_convolve,
                            ],
                            int(best_convolve),
                        )
                        convolved_ndays = pss.calc_harmonic_sums(
                            pspec_meta_data.freq_bin_weights,
                            List(prep_convolve[0]),
                            List(prep_convolve[1]),
                            List([int(best_nharm)]),
                        )
                        for array_dm_index, dm_index in enumerate(
                            range(dm_idx_min_sigma, dm_idx_max_sigma)
                        ):
                            powers = pss.calc_harmonic_sums(
