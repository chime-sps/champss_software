...
def main(
    start_date, end_date, x_axis_key, y_axis_key, colouring_key
):
    """Entry function to query and plot Processes MongoDB collection for cAdvisor metrics."""
    client = MongoClient(host="sps-archiver", port=27017)
    db = client["sps-processing"]
    collection = db["processes"]

    processes = list(collection.find({"datetime": {"$gte": start_date, "$lte": end_date}, "status": {"$eq": 2}}))

    plot_data = pd.DataFrame(processes)

    plot_title = f"{x_axis_key}_vs_{y_axis_key}_by{colouring_key}"

    plot = plot_data.plot.scatter(x=x_axis_key, y=y_axis_key, c=plot_data[colouring_key], colormap="viridis", title=plot_title)
