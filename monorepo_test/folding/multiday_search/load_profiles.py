...
def find_central_obs(directory, F0, DM, message=True):
        ...
        central_obs_date = dt.date.fromordinal(int(np.median([date.toordinal() for date in dates])))
        if message:
            print(f"Oldest observation is {oldest_obs_date}")
            print(f"Center observation is {central_obs_date}, referencing ephemeris to this date")
        #first_obs_par = f"{directory}/cand_{round(DM, 2)}_{round(F0, 2)}_" + oldest_obs_date.strftime('%Y-%m-%d') + ".par"
        central_obs_par = f"{directory}/cand_{round(DM, 2)}_{round(F0, 2)}_" + central_obs_date.strftime('%Y-%m-%d') + ".par"

        return central_obs_par

def load_profiles(directory, F0, DM, RA, DEC, load_only=False, max_npbin=256):
    """
    Finds central observation and uses its ephemeris as the reference epoch for all other archives
    ...
    if not load_only: 
        ...
        subprocess.run(["pam", "-E", central_obs_par, "-e", ".newar.FT", 
                        f"cand*{round(DM,2)}*{round(F0,2)}*-??.FT"], cwd=directory)
    ...
    if np.unique(PEPOCHs).size > 1:
        print("WARNING: not all profiles references to the same PEPOCHs, re-apply same ephemeris to all archives")
    ...
    if npbin > max_npbin:
        ...
        profs = profs.reshape(profs.shape[0], max_npbin, profs.shape[1]//max_npbin).sum(2)
        npbin = max_npbin
    ...
    param_dict = dict({'profiles': profs, 'times': times, 'F0': F0,'DM': DM,'RA': RA,'DEC': DEC, 
                       'directory': directory,'npbin': npbin, 'T': Tmax_from_reference})
    return param_dict
